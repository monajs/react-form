...
function (_Component) {
  ...
  function Form() {
    ...
    _defineProperty(_assertThisInitialized(_this), "cancelWatcher", function (bn) {
      ...
      _this.verifyHandler(bn, true);
    });

    _defineProperty(_assertThisInitialized(_this), "subscibeReport", function (bn, vm) {
      if (_this.itemMap.has(bn)) {
        !_util.isProd && _util.log.warn("Encountered two form member with the same `bn`, `bn`: \"".concat(bn, "\" should be unique so that form collect their value correctly."), 'https://github.com/monajs/react-form/issues/7');
        return false;
      }
    ...
    _defineProperty(_assertThisInitialized(_this), "getVerifyInfo", function () {
      ...
      _this.verifyBnWithOrder.forEach(function (verifyItem) {
        var verifyInfo = Reflect.has(verifyContextValue, verifyItem.key) ? verifyContextValue[verifyItem.key] : verifyItem.vm.verifyHandler(verifyItem.vm.state.val);
        if ((0, _util.isPlainObject)(verifyInfo)) verifyResList.push(verifyInfo);
      });
    ...
    _defineProperty(_assertThisInitialized(_this), "verifyHandler", function (bn, verifyInfo) {
      ...
      _this.setState(function (prevState) {
        var _prevState$verifyCont = prevState.verifyContextValue,
            verifyContextValue = _prevState$verifyCont === void 0 ? {} : _prevState$verifyCont;
        ...
        return {
          verifyContextValue: _objectSpread({}, verifyContextValue)
        };
      });
    });
