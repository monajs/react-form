...
var withFormContext = function withFormContext(WrappedComponent) {
  ...
  function (_Component) {
    ...
    function FormItemComponent(props) {
      ...
      (0, _defineProperty2["default"])((0, _assertThisInitialized2["default"])(_this), "devWarning", function () {
        if (!_this.bn) {
          var CompInfo = "{Component: `".concat(WrappedComponent.name, "`, id: ").concat(id, "}");
          !_util.isProd && _util.log.warn("The prop `bn` is required for form member (".concat(CompInfo, "), but its value is `undefined`."));

          if (_this.verify || _this.verifyMsg) {
            !_util.isProd && _util.log.warn("The prop `bn` is missed with Component\uFF08".concat(CompInfo, "\uFF09.If you want to use original Component, then the prop `verify` and `verifyMsg` is needless."));
          }
        }
